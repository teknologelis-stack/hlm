...
if (empty($id)) {
    jsonResponse(['success' => false, 'message' => 'IP adresi ID gereklidir']);
}

try {
    $mikrotik = new MikroTikHelper($deviceId);
    
    if ($mikrotik->removeIPAddress($id)) {
        logAction('ip_address_deleted', "IP adresi silindi: {$id}", $mikrotik->device['id'] ?? null);
        jsonResponse([
            'success' => true,
    ...
    error_log("IP address delete error: " . $e->getMessage());
    jsonResponse([
        'success' => false,
