<?php
/**
 * API: MikroTik IP Address Management
 */
...
try {
    ...
    if ($method === 'POST') {
        ...
        if (empty($address) || empty($interface)) {
            echo json_encode(['success' => false, 'error' => 'Adres ve Arayüz gerekli']);
            exit;
        }
        ...
        echo json_encode(['success' => true, 'message' => 'IP Adresi eklendi']);
        exit;
    }
    ...
    if ($method === 'PUT') {
        ...
        $client->query($query)->read();
        echo json_encode(['success' => true, 'message' => 'IP Adresi güncellendi']);
        exit;
    }
    ...
    if ($method === 'DELETE') {
        ...
        echo json_encode(['success' => true, 'message' => 'IP Adresi silindi']);
        exit;
    }
    
} catch (Exception $e) {
    error_log("[API:mikrotik-ip] Error: " . $e->getMessage());
